@php
    ...
    if (!is_null($term)) {
        $searchQuery = 'term=' . request()->input('term');
    }
@endphp
        ...
        <div class="right-content">
            ...
            <ul class="right-content-menu">
                ...
                @php
                    ...
                    $compareRoute = auth()->user() ? route('shop.customer.product.compare') : route('shop.product.compare');
                @endphp

                @if ($showCompare)
                    <li class="compare-dropdown-container">
                        <a href="{{ $compareRoute }}" style="color: #242424;">
                            ...
                            @foreach (core()->getCurrentChannel()->currencies as $currency)
                                <li>
                                    @if (isset($searchQuery))
                                        <a href="?{{ $searchQuery }}&currency={{ $currency->code }}">{{ $currency->code }}</a>
                                    @else
                        ...
                        <ul class="dropdown-list account guest">
                            <li>
                                ...
                                <div style="margin-top: 15px;">
                                    <a class="btn btn-primary btn-md" href="{{ route('shop.customer.session.index') }}"
                                       style="color: #ffffff">
                                        {{ __('shop::app.header.sign-in') }}
                                    </a>

                                    <a class="btn btn-primary btn-md" href="{{ route('shop.customer.register.index') }}"
                        ...
                        <ul class="dropdown-list account customer">
                            <li>
                                ...
                                <ul>
                                    ...
                                    <li>
                                        <a href="{{ route('shop.customer.session.destroy') }}">{{ __('shop::app.header.logout') }}</a>
                                    </li>
                                </ul>
                            </li>
                        </ul>
...
@push('scripts')
        ...
        Vue.component('image-search-component', {
            ...
            methods: {
                uploadImage: function () {
                    let imageInput = this.$refs.image_search_input;
    ...
    <script>
        $(document).ready(function () {
            ...
            @auth('customer')
                @php
                    $compareCount = app('BagistoPackages\Shop\Repositories\CustomerCompareProductRepository')
                        ->count([
