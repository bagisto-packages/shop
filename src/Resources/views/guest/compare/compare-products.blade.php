@php
    $attributeRepository = app('\BagistoPackages\Shop\Repositories\AttributeFamilyRepository');
                ...
                <template v-if="isProductListLoaded && products.length > 0">
                    @php
                        ...
                        array_splice($comparableAttributes, 1, 0, [[
                            'code' => 'product_image',
                            'admin_name' => __('shop::app.customer.compare.product_image'),
                        ]]);

                        array_splice($comparableAttributes, 2, 0, [[
                            'code' => 'addToCartHtml',
                            'admin_name' => __('shop::app.customer.compare.actions'),
                        ]]);
                    @endphp
                        ...
                        <tr>
                            ...
                            <td :key="`title-${index}`" v-for="(product, index) in products">
                                @switch ($attribute['code'])
                                    @case('name')
                                    <a :href="`${baseUrl}/${product.url_key}`"
                                       class="unset remove-decoration active-hover">
                                    ...
                                    <a :href="`${baseUrl}/${product.url_key}`" class="unset">
                                        <img
                                            ...
                                            :onerror="`this.src='${baseUrl}/vendor/packages/ui/assets/images/product/large-product-placeholder.png'`"
                                            alt=""/>
                                    </a>
                                    @break
                                    ...
                                    <div class="action">
                                        ...
                                        <span class="icon white-cross-sm-icon remove-product"
                                              @click="removeProductCompare(product.id)"></span>
                                    </div>
                                    @break
                                    ...
                                    @switch ($attribute['type'])
                                        ...
                                        <span
                                            v-text="product.product['{{ $attribute['code'] }}']
                                                            ? '{{ __('shop::app.shop.general.yes') }}'
                                                            : '{{ __('shop::app.shop.general.no') }}'"
                                        ></span>
                                        @break;

                                        @case('checkbox')
                                        <span v-if="product.product['{{ $attribute['code'] }}']"
                                              v-html="getAttributeOptions(product['{{ $attribute['code'] }}'] ? product : product.product['{{ $attribute['code'] }}'] ? product.product : null, '{{ $attribute['code'] }}', 'multiple')"
                                              class="fs16"></span>
                                        ...
                                        @case('select')
                                        <span v-if="product.product['{{ $attribute['code'] }}']"
                                              v-html="getAttributeOptions(product['{{ $attribute['code'] }}'] ? product : product.product['{{ $attribute['code'] }}'] ? product.product : null, '{{ $attribute['code'] }}', 'single')"
                                              class="fs16"></span>
                                        <span v-else class="fs16">__</span>
                                        @break;
                                        ...
                                        @default
                                        <span
                                            v-html="product['{{ $attribute['code'] }}'] ? product['{{ $attribute['code'] }}'] : product.product['{{ $attribute['code'] }}'] ? product.product['{{ $attribute['code'] }}'] : '__'"
                                            class="fs16"></span>
                                        @break;
                                    @endswitch
                    ...
                    if (this.isCustomer) {
                        ...
                        if (productId === "all") {
                            ...
                            window.flashMessages = [{
                                'type': 'alert-success',
                                'message': '{{ __('shop::app.customer.compare.removed-all') }}'
                            }];
                        } else {
                            ...
                            window.flashMessages = [{
                                'type': 'alert-success',
                                'message': '{{ __('shop::app.customer.compare.removed') }}'
                            }];
                        }
