            ...
            <div class="sample-list">
                ...
                <ul>
                    @foreach ($product->downloadable_samples as $sample)
                        <li>
                            <a href="{{ route('shop.downloadable.download_sample', ['type' => 'sample', 'id' => $sample->id]) }}"
                               target="_blank">
                                {{ $sample->title }}
                            </a>
                        </li>
            ...
            <div class="link-list control-group" :class="[errors.has('links[]') ? 'has-error' : '']">
                ...
                <ul>
                    @foreach ($product->downloadable_links as $link)
                        <li>
                            <span class="checkbox">
                                <input type="checkbox" name="links[]" v-validate="'required'" value="{{ $link->id }}"
                                       id="{{ $link->id }}"
                            ...
                            @if ($link->sample_file || $link->sample_url)
                                <a href="{{ route('shop.downloadable.download_sample', ['type' => 'link', 'id' => $link->id]) }}"
                                   target="_blank">
