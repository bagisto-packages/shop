@if (request()->route()->getName() == 'shop.checkout.onepage.index')
    <script
        src="https://www.paypal.com/sdk/js?client-id={{core()->getConfigData('sales.paymentmethods.paypal_smart_button.client_id')}}"
            ...
            eventBus.$on('after-payment-method-selected', function (payment) {
                ...
                if (typeof paypal == 'undefined') {
                    window.flashMessages = [{
                        'type': 'alert-error',
                        'message': "SDK Validation error: 'client-id not recognized for either production or sandbox: {{core()->getConfigData('sales.paymentmethods.paypal_smart_button.client_id')}}'"
                    }];
                ...
                let options = {
                    ...
                    createOrder: function (data, actions) {
                        return window.axios.get("{{ route('shop.paypal.smart_button.details') }}")
                    ...
                    onApprove: function (data, actions) {
                        ...
                        return actions.order.capture()
                            ...
                            .then(function (details) {
                                return window.axios.post("{{ route('shop.paypal.smart_button.save_order') }}", {
                                    '_token': "{{ csrf_token() }}",
                                    'data': details
                                })
                                    .then(function (response) {
