    ...
    @push('scripts')
        ...
        <script type="text/x-template" id="bundle-option-item-template">
            <div class="bundle-option-item">
                <div class="control-group"
                     :class="[errors.has('bundle_options[' + option.id + '][]') ? 'has-error' : '']">
                    <label :class="[option.is_required ? 'required' : '']">@{{ option.label }}</label>

                    <div v-if="option.type == 'select'">
                        <select class="control" :name="'bundle_options[' + option.id + '][]'" v-model="selected_product"
                                v-validate="option.is_required ? 'required' : ''" :data-vv-as="option.label + '&quot;'">
                    ...
                    <div v-if="option.type == 'radio'">
                        <span class="radio" v-if="! option.is_required">
                            <input type="radio" :name="'bundle_options[' + option.id + '][]'" v-model="selected_product"
                                   value="0" :id="'bundle_options[' + option.id + '][]'">
                        ...
                        <span class="radio" v-for="(product, index2) in option.products">
                            <input type="radio" :name="'bundle_options[' + option.id + '][]'" v-model="selected_product"
                                   ...
                                   :data-vv-as="'&quot;' + option.label + '&quot;'" :value="product.id"
                    ...
                    <div v-if="option.type == 'checkbox'">
                        <span class="checkbox" v-for="(product, index2) in option.products">
                            <input type="checkbox" :name="'bundle_options[' + option.id + '][]'" :value="product.id"
                                   v-model="selected_product" v-validate="option.is_required ? 'required' : ''"
                    ...
                    <div v-if="option.type == 'multiselect'">
                        <select class="control" :name="'bundle_options[' + option.id + '][]'" v-model="selected_product"
                                v-validate="option.is_required ? 'required' : ''"
                                :data-vv-as="'&quot;' + option.label + '&quot;'" multiple>
        ...
        <script>
            Vue.component('bundle-option-list', {
                ...
                data: function () {
                    return {
                        config: @json(app('BagistoPackages\Shop\Helpers\BundleOption')->getBundleConfig($product)),
